...
var curso = window.document.getElementById('curso')
...
function adicionar_usuario () {
    ...
    const usuario = { nome: `${nome.value}`, idade: `${idade.value}`, curso: `${curso.value}` };
...
function altera_usuario() {
    ...
    curso.value = db_usuario[indice].curso
...
function salvar_usuario (aux) {
    ...
    db_usuario[aux].curso = curso.value
...
function botao_alterar () {
 ...
 function exclui_usuario () {
    ...
    //Remove um elemento (nome e idade) de acordo com o indice na variável (db_usuario)
    db_usuario.splice(indice, 1);
